window.addEventListener("load", () => {
  ...
  const bridges = [
    { trigger: "#s2", from: "#ebe4d9", to: "#68190f", shape: "circle", x: 50, y: 88, scale: 7.5 },
    { trigger: "#s3", from: "#68190f", to: "#f3eee6", shape: "plane", x: 50, y: 78, scale: 1 },
    { trigger: "#s4", from: "#f3eee6", to: "#080808", shape: "circle", x: 52, y: 72, scale: 8 },
    { trigger: "#s5", from: "#080808", to: "#d9d0c3", shape: "plane", x: 50, y: 76, scale: 1 },
    { trigger: "#s6", from: "#d9d0c3", to: "#68190f", shape: "circle", x: 68, y: 58, scale: 7.5 },
    { trigger: "#s7", from: "#68190f", to: "#f3eee6", shape: "plane", x: 50, y: 76, scale: 1 },
    { trigger: "#s8", from: "#b06c1b", to: "#d08920", shape: "gold", x: 72, y: 54, scale: 3.2 },
    { trigger: "#s9", from: "#d08920", to: "#d7cdbf", shape: "paper", x: 70, y: 53, scale: 3.6 },
    { trigger: "#s10", from: "#d7cdbf", to: "#100403", shape: "circle", x: 68, y: 48, scale: 7.5 }
  ];
  ...
  const resetShape = config => {
    ...
    gsap.set(orb, {
      ...
      scale: config.shape === "gold" ? .32 : .12,
      rotation: config.shape === "paper" ? -9 : 0
    });
    gsap.set(plane, { backgroundColor: config.to, scaleY: .05, opacity: 0, rotation: 0 });
  ...
  bridges.forEach(config => {
    ...
    timeline.call(() => resetShape(config), null, 0)
      .fromTo(wash,
        ...
        { opacity: .34, backgroundColor: config.to, duration: .62, ease: "none", immediateRender: false }, 0);

    if (config.shape === "plane") {
      timeline.fromTo(plane,
        { opacity: 0, scaleY: .05 },
        { opacity: .9, scaleY: 1.55, duration: .76, ease: "none", immediateRender: false }, .02);
    } else {
      timeline.fromTo(orb,
        { opacity: 0, scale: config.shape === "gold" ? .32 : .12 },
        { opacity: config.shape === "gold" ? .88 : .94, scale: config.scale, borderRadius: config.shape === "paper" ? "2px" : "50%", rotation: 0, duration: .76, ease: "none", immediateRender: false }, .02);
    }

    timeline.fromTo(axis,
      { opacity: 0, scaleY: .3 },
      { opacity: .18, scaleY: 1, duration: .45, ease: "none", immediateRender: false }, .12)
      .to([wash, orb, plane, axis], { opacity: 0, duration: .22, ease: "none" }, .78)
      .to(layer, { autoAlpha: 0, duration: .01 }, .995);
  });
